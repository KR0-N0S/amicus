 ...
 */
exports.requireRole = (allowedRoles, requireOrganization = true) => {
  return (req, res, next) => {
    try {
      ...
      const userOrg = userOrganizations.find(org => org.id.toString() === organizationId.toString());
 ...
 */
exports.requireOrganizationMembership = () => {
  return (req, res, next) => {
    try {
      // Sprawdź czy użytkownik jest zalogowany
      if (!req.userId || !req.user) {
