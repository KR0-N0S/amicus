...
export const checkOrganizationAccess = async (req: Request, res: Response, next: NextFunction): Promise<void | Response> => {
  try {
    ...
    const userBelongsToOrg = req.userOrganizations.some(
      org => org.id === parseInt(organizationId as string, 10) || org.id === organizationId
    );
    ...
    if (requiredRoles.length > 0) {
      const hasRequiredRole = req.userOrganizations.some((org: any) => {
        return (org.id === parseInt(organizationId as string, 10) || org.id === organizationId) && 
               org.role && 
