 ...
 */
export const requireModule = (requiredModules: string | string[]) => {
 ...
 */
export const requireRoleAndModule = (allowedRoles: string | string[], requiredModules: string | string[]) => {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      // Najpierw sprawdź rolę
      const roleMiddleware = require('./roleMiddleware');
      roleMiddleware.requireRole(allowedRoles)(req, res, (roleErr?: any) => {
        if (roleErr) return next(roleErr);
        
        // Następnie sprawdź dostęp do modułu
        requireModule(requiredModules)(req, res, next);
      });
    } catch (error) {
 ...
 */
export const requireModuleFeature = (moduleCode: string, featureKey: string) => {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      // Najpierw sprawdź dostęp do modułu
      requireModule(moduleCode)(req, res, (moduleErr?: any) => {
